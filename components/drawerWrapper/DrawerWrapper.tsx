...
export default function DrawerWrapper({ onSelect, svgdata }: Params) {
    ...
    const paginationRef = useRef<Pagination>({
        pagenumber: 1, originalList: Object.keys(icons)
            .filter(i => i.includes('Icon'))
    })
    ...
    function resetList() {
        const value = paginationRef.current.originalList.filter(i => i.toLowerCase().includes(inputValue.toLowerCase()))
            .slice(0, MAX_COUNT * 1)
        setList(value)
    ...
    function handleScroll(event: React.UIEvent<HTMLDivElement>) {
        ...
        if (target.scrollHeight - target.scrollTop === target.clientHeight) {
            paginationRef.current.pagenumber = (paginationRef.current?.pagenumber || 0) + 1
            setList(paginationRef.current.originalList.filter(i => i.toLowerCase().includes(inputValue.toLowerCase()))
                .slice(0, MAX_COUNT * paginationRef.current?.pagenumber))
        }
    }
        ...
        <>
            <div className=" mb-4 flex justify-between">
                    ...
                    <TooltipProvider>
                        <Tooltip>
                            <TooltipTrigger asChild>
                                <Button variant="outline" className="p-2" onClick={() => setOpen(!open)}>
                                    <Plus />
                                </Button>
                            </TooltipTrigger>
                            <TooltipContent>
                ...
                {svgdata?._svg && <DropdownMenu>
                    ...
                    <DropdownMenuContent>
                        <DropdownMenuItem onClick={() => downloadSvg(filename, 'svg')}>Download SVG</DropdownMenuItem>
                        <DropdownMenuItem onClick={() => downloadPng(filename, 'png')}>Download PNG</DropdownMenuItem>
                    </DropdownMenuContent>
                </DropdownMenu>}
            </div >
                ...
                <DrawerContent>
                    <div className="container py-2">
                        <div className="flex justify-between items-center">
                            <h3 className="text-base-content text-lg font-bold">Pick an icon</h3>
                                    ...
                                    if (Component?.render?.displayName)
                                        return (
                                            <div key={i} onClick={() => handleSelect(i)}
                                                ...
                                                <Component
                                                    ref={(e) => svgRef.current ? svgRef.current[i] = e : svgRef.current = { [i]: e }}
                                                />
                                            </div>
                                        )
                                    return null

...
export default function Controls(svgdata: any) {
  const [controler, setControler] = useState<any>(() => _controler());
  const [color, setColor] = useState(() => _controler()?.bgColor?.attr?.value);
  const controlerRef = useRef<HTMLDivElement>(null);

  function handleControler(value: string | number, id: string) {
    setControler((prev: any) => {
      return {
        ...prev,
        [id]: { ...prev[id], attr: { ...prev[id].attr, value } },
      };
    });
  }
  ...
  function controlComponent(value: any, key: string) {
    ...
    switch (attr.type) {
      case "rgba_color":
        ...
        const width = controlerRef.current?.getBoundingClientRect()?.width;
        return (
          <ColorPicker
            {...rest}
            width={width}
